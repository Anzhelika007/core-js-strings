...
function countVowels(str) {
  ...
  const answer = [...str].filter((letter) => vowels.includes(letter));
...
function isPalindrome(str) {
  const newStr = str
    ...
    .replaceAll('?', '')
    .toLowerCase();
  const sub = Math.floor(newStr.length / 2);
  if (sub === 0 && str.length === 1) {
    return true;
  }

  const subStart = newStr.slice(0, sub);
  const subEnd = [...newStr.slice(-sub)].reverse().join('');
...
function invertCase(str) {
  ...
  const answer = [...str]
    .map((el) =>
      el.charCodeAt() >= upperStart && el.charCodeAt() <= upperEnd
        ? el.toLowerCase()
        : el.toUpperCase()
    )
    .join('');
...
function extractNameFromTemplate(value) {
  const answer = value.replace('Hello, ', '').replace('!', '');
  return answer;
}
  ...
  return map
    .call(str, (x) => {
      if (x.charCodeAt(0) > 64 && x.charCodeAt(0) < 91) {
        ...
        return String.fromCharCode(x.charCodeAt(0) + 13 - 90 + 64);
      }
      if (x.charCodeAt(0) > 96 && x.charCodeAt(0) < 123) {
        if (x.charCodeAt(0) + 13 < 123) {
          return String.fromCharCode(x.charCodeAt(0) + 13);
        }
        return String.fromCharCode(x.charCodeAt(0) + 13 - 122 + 96);
      }
